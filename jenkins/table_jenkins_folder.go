...
func tableJenkinsFolder() *plugin.Table {
	return &plugin.Table{
		...
		Columns: []*plugin.Column{
			...
			{Name: "full_display_name", Type: proto.ColumnType_STRING, Description: "Human readable name of the folder, including folder."},
			{Name: "full_name", Type: proto.ColumnType_STRING, Description: "Folder name including folder."},
			{Name: "name", Type: proto.ColumnType_STRING, Description: "Unique key for the folder."},
...
func getJenkinsFolder(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	folderName := d.KeyColumnQuals["name"].GetStringValue()

	// Empty check for folderName
	if folderName == "" {
		return nil, nil
	}
	...
	folder, err := client.GetJob(ctx, folderName)
	if err != nil {
